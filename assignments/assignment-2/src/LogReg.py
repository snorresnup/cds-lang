...
def main():
    tracker = EmissionsTracker(project_name=f"a2_logreg_emissions", 
        experiment_id=f"a2_logreg_emissions",
        output_dir=os.path.join("out","emissions"),
        output_file=f"a2_logreg_emissions.csv")
    
    tracker.start()
    
    tracker.start_task("a2_load_data")
    X_train, X_test, y_train, y_test = load_data()
    tracker.stop_task()

    tracker.start_task("a2_vectorize_data")
    X_train_feats, X_test_feats = vectorizer(X_train, X_test)
    tracker.stop_task()

    tracker.start_task("a2_train_logreg_classifier")
    classifier, y_pred = create_classifier(X_train_feats, y_train, X_test_feats)
    tracker.stop_task()

    tracker.start_task("a2_classification_report")
